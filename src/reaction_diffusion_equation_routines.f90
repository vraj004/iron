...
#include "mpif.h"
  ...
  !PUBLIC REACTION_DIFFUSION_CONTROL_LOOP_PRE_LOOP
      ...
      IF(ASSOCIATED(SOLVERS)) THEN
        ...
        IF(ASSOCIATED(CONTROL_LOOP)) THEN
          ...
          IF(ASSOCIATED(PROBLEM)) THEN
            SELECT CASE(PROBLEM%SPECIFICATION(3))
            CASE(PROBLEM_CELLML_REAC_INTEG_REAC_DIFF_STRANG_SPLIT_SUBTYPE, &
              & PROBLEM_STRANG_REACTION_DIFFUSION_ELASTICITY_SUBTYPE, &
      ...
      IF(ASSOCIATED(SOLVER)) THEN
        IF(ASSOCIATED(CONTROL_LOOP%PROBLEM)) THEN 
          ...
          SELECT CASE(CONTROL_LOOP%PROBLEM%SPECIFICATION(3))
            CASE(PROBLEM_CELLML_REAC_INTEG_REAC_DIFF_STRANG_SPLIT_SUBTYPE, &
              & PROBLEM_STRANG_REACTION_DIFFUSION_ELASTICITY_SUBTYPE,PROBLEM_STRANG_REAC_DIFF_GROWTH_ELASTIC_SUBTYPE)
              SELECT CASE(SOLVER%GLOBAL_NUMBER)
                CASE(1)
    ...
    IF(ASSOCIATED(CONTROL_LOOP)) THEN
      IF(ASSOCIATED(SOLVER)) THEN
        IF(ASSOCIATED(CONTROL_LOOP%PROBLEM)) THEN
          ...
          SELECT CASE(CONTROL_LOOP%PROBLEM%SPECIFICATION(3))
            CASE(PROBLEM_CELLML_REAC_INTEG_REAC_DIFF_STRANG_SPLIT_SUBTYPE, &
              & PROBLEM_CONSTANT_REAC_DIFF_NO_SPLIT_SUBTYPE,PROBLEM_STRANG_REACTION_DIFFUSION_ELASTICITY_SUBTYPE, &
              & PROBLEM_STRANG_REAC_DIFF_GROWTH_ELASTIC_SUBTYPE)
              CALL CONTROL_LOOP_CURRENT_TIMES_GET(CONTROL_LOOP,CURRENT_TIME,TIME_INCREMENT,ERR,ERROR,*999)
    ...
    IF(ASSOCIATED(CONTROL_LOOP)) THEN
      ...
      IF(ASSOCIATED(PROBLEM)) THEN
        ...
        CASE(PROBLEM_STRANG_REACTION_DIFFUSION_ELASTICITY_SUBTYPE)
          WRITE(*,*) 'I AM HERE AT END OF EACH LOOP OF THE CONTROL LOOP FOR REACTION DIFFUSION'
          !At the end of an RD control-loop evaluate spatial distribution of F-actin
          
          !take F-actin and evaluate a growth tensor or a force distribution. 
          
          
          !update a field that will be mapped to growth tensor field in mechanics.
