      ...
      IF(SIZE(problemSpecification,1)==3) THEN
        ...
        CASE(PROBLEM_STRANG_REACTION_DIFFUSION_ELASTICITY_SUBTYPE)
          localError="The third problem specification of "//TRIM(NumberToVstring(problemSubtype,"*",err,error))// &
            & " has only been implemented to couple the RD and mechanics control loops."
          CALL FlagError(localError,err,error,*999)
          !ok
        CASE(PROBLEM_STRANG_REAC_DIFF_GROWTH_ELASTIC_SUBTYPE)
          !ok
        CASE DEFAULT
          localError="The third problem specification of "//TRIM(NumberToVstring(problemSubtype,"*",err,error))// &
...
999 ERRORS("ReacDiffFiniteElastic_ProblemSpecificationSet",err,error)
  ...
  SUBROUTINE REAC_DIFF_FINITE_ELASTIC_PROBLEM_SETUP(PROBLEM,PROBLEM_SETUP,ERR,ERROR,*)
    ...
    IF(ASSOCIATED(PROBLEM)) THEN
      ...
      !--------------------------------------------------------------------
      !   Transient Strang-split reaction diffusion, simple finite elasticity  or growth finite elasticity 
      !--------------------------------------------------------------------
      CASE(PROBLEM_STRANG_REACTION_DIFFUSION_ELASTICITY_SUBTYPE, &
        ...
        CASE(PROBLEM_SETUP_CONTROL_TYPE)
          ...
          CASE(PROBLEM_SETUP_START_ACTION)
            ...
            IF(PROBLEM_SUBTYPE==PROBLEM_STRANG_REACTION_DIFFUSION_ELASTICITY_SUBTYPE) THEN
            ...
            ELSEIF(PROBLEM_SUBTYPE==PROBLEM_STRANG_REAC_DIFF_GROWTH_ELASTIC_SUBTYPE) THEN
              CALL CONTROL_LOOP_LABEL_SET(ELASTICITY_SUB_LOOP,'ELASTICITY_TIME_LOOP',ERR,ERROR,*999)
              CALL CONTROL_LOOP_TYPE_SET(ELASTICITY_SUB_LOOP,PROBLEM_CONTROL_TIME_LOOP_TYPE,ERR,ERROR,*999)
            ENDIF
        ...
        CASE(PROBLEM_SETUP_SOLVERS_TYPE)
          ...
          CASE(PROBLEM_SETUP_START_ACTION)
            ...
            CALL SOLVER_LABEL_SET(SOLVER,"First RD ODE Solver",ERR,ERROR,*999)
            ...
            CALL SOLVER_LABEL_SET(SOLVER,"Parabolic RD solver",ERR,ERROR,*999)
            ...
            CALL SOLVER_TYPE_SET(SOLVER,SOLVER_DAE_TYPE,ERR,ERROR,*999)
            CALL SOLVER_LABEL_SET(SOLVER,"Second RD ODE Solver",ERR,ERROR,*999)
            CALL SOLVER_LIBRARY_TYPE_SET(SOLVER,SOLVER_CMISS_LIBRARY,ERR,ERROR,*999)
            ...
            ELSEIF(PROBLEM_SUBTYPE==PROBLEM_STRANG_REAC_DIFF_GROWTH_ELASTIC_SUBTYPE) THEN
              NULLIFY(SOLVER)
              CALL SOLVERS_NUMBER_SET(ELASTICITY_SOLVERS,2,ERR,ERROR,*999)
              CALL SOLVERS_SOLVER_GET(ELASTICITY_SOLVERS,1,SOLVER,ERR,ERROR,*999)
              CALL SOLVER_LABEL_SET(SOLVER,"Mechanics ODE Solver",ERR,ERROR,*999)
              CALL SOLVER_TYPE_SET(SOLVER,SOLVER_DAE_TYPE,ERR,ERROR,*999)
              !Set solver defaults
              CALL SOLVER_LIBRARY_TYPE_SET(SOLVER,SOLVER_CMISS_LIBRARY,ERR,ERROR,*999)
              !Set the finite elasticity solver to be a nonlinear solver
              NULLIFY(SOLVER)
              CALL SOLVERS_SOLVER_GET(ELASTICITY_SOLVERS,2,SOLVER,ERR,ERROR,*999)
              CALL SOLVER_TYPE_SET(SOLVER,SOLVER_NONLINEAR_TYPE,ERR,ERROR,*999)
              CALL SOLVER_LIBRARY_TYPE_SET(SOLVER,SOLVER_PETSC_LIBRARY,ERR,ERROR,*999)
              CALL SOLVER_LABEL_SET(SOLVER,"Mechanics PDE solver",ERR,ERROR,*999)
        ...
        CASE(PROBLEM_SETUP_CELLML_EQUATIONS_TYPE)
          ...
          CASE(PROBLEM_SETUP_START_ACTION)
            ...
            IF(PROBLEM_SUBTYPE==PROBLEM_STRANG_REAC_DIFF_GROWTH_ELASTIC_SUBTYPE) THEN
              CALL CONTROL_LOOP_SUB_LOOP_GET(CONTROL_LOOP,2,ELASTICITY_SUB_LOOP,ERR,ERROR,*999)
              CALL CONTROL_LOOP_SOLVERS_GET(ELASTICITY_SUB_LOOP,ELASTICITY_SOLVERS,ERR,ERROR,*999)
        ...
        CASE DEFAULT
          LOCAL_ERROR="The setup type of "//TRIM(NUMBER_TO_VSTRING(PROBLEM_SETUP%SETUP_TYPE,"*",ERR,ERROR))// &
            & " is invalid for a reaction diffusion finite elasticity equation."
          CALL FlagError(LOCAL_ERROR,ERR,ERROR,*999)
        END SELECT

      CASE DEFAULT
        LOCAL_ERROR="The problem subtype of "//TRIM(NUMBER_TO_VSTRING(PROBLEM%SPECIFICATION(3),"*",ERR,ERROR))// &
          & " is not implemented as a subtype of the reaction diffusion mechanics coupling."
        CALL FlagError(LOCAL_ERROR,ERR,ERROR,*999)
      END SELECT
    ELSE
      CALL FlagError("Problem is not associated.",ERR,ERROR,*999)
      ...
      IF(ASSOCIATED(SOLVER)) THEN
        IF(ASSOCIATED(CONTROL_LOOP%PROBLEM)) THEN
          ...
          CASE(PROBLEM_STRANG_REACTION_DIFFUSION_ELASTICITY_SUBTYPE)
            ...
            END SELECT
          CASE(PROBLEM_STRANG_REAC_DIFF_GROWTH_ELASTIC_SUBTYPE)
            SELECT CASE(CONTROL_LOOP%SUB_LOOP_INDEX) 
            CASE(1)
              !RD subloop
              CALL REACTION_DIFFUSION_PRE_SOLVE(SOLVER,ERR,ERROR,*999)
            CASE(2)
              !growth mechanics loop
      ...
      IF(ASSOCIATED(SOLVER)) THEN
        IF(ASSOCIATED(CONTROL_LOOP%PROBLEM)) THEN 
          ...
          CASE(PROBLEM_STRANG_REACTION_DIFFUSION_ELASTICITY_SUBTYPE, &
            ...
            WRITE(*,*) 'HELLO FROM POST_SOLVE OF MULTI_PHYS PROBLEM CLASS ROUTINE'
              CALL REACTION_DIFFUSION_POST_SOLVE(CONTROL_LOOP,SOLVER,ERR,ERROR,*999)
        ...
        IF(ASSOCIATED(PROBLEM)) THEN
          ...
          CASE(PROBLEM_REACTION_DIFFUSION_FINITE_ELASTICITY_TYPE)
            ...
            CASE(PROBLEM_CONTROL_TIME_LOOP_TYPE)
              ...
              CASE(PROBLEM_STRANG_REACTION_DIFFUSION_ELASTICITY_SUBTYPE, &
                   PROBLEM_STRANG_REAC_DIFF_GROWTH_ELASTIC_SUBTYPE)
                !CALL REACTION_DIFFUSION_CONTROL_LOOP_PRE_LOOP()
      ...
      IF(CONTROL_LOOP%NUMBER_OF_SUB_LOOPS==0) THEN
        ...
        IF(ASSOCIATED(PROBLEM)) THEN
          ...
          CASE(PROBLEM_CONTROL_LOAD_INCREMENT_LOOP_TYPE)
            ...
            CALL FiniteElasticity_ControlLoadIncrementLoopPostLoop(CONTROL_LOOP,ERR,ERROR,*999)
          CASE DEFAULT
            !do nothing
          END SELECT
        ELSE
                ...
                IF(ASSOCIATED(SOLVER_MAPPING)) THEN
                  DO equations_set_idx=1,SOLVER_MAPPING%NUMBER_OF_EQUATIONS_SETS
                    ...
                    IF(ASSOCIATED(EQUATIONS_SET)) THEN
                      ...
                      FILENAME="MainTime_FE"//TRIM(NUMBER_TO_VSTRING(DEPENDENT_REGION%USER_NUMBER,"*",ERR,ERROR))// &
